...
def parse_vacuum_level(subfolder):
    """Membaca potensial vakum dari pot_1d.dat atau pp.out"""
    p1d = os.path.join(subfolder, "pot_1d.dat")
    pp_out = os.path.join(subfolder, "pp.out")
    target = p1d if os.path.exists(p1d) else (pp_out if os.path.exists(pp_out) else None)
    if not target:
        return 0.0
    try:
        data = []
        ...
        if len(data) > 0:
            arr = np.array(data)
            # Sumbu z dinormalisasi oleh pp.x: z_frac in [0.0, 1.0]
            # Monolayer berada di dekat batas sel (z ~ 0 dan z ~ c),
            # sedangkan plateau vakum datar berada di tengah sel (z_frac ~ 0.40 - 0.60)
            vac_mask = (arr[:, 0] >= 0.40) & (arr[:, 0] <= 0.60)
            if np.any(vac_mask):
                return float(np.mean(arr[vac_mask, 1]))
...
for d_idx, d in enumerate(directions):
    ...
    # 3. Hitung Waktu Relaksasi tau dan Mobilitas mu
    ...
    ax_e.set_title(f'ZrFCl Elastic Energy ({d})')
    ...
    ax_b.set_title(f'ZrFCl Deformation Potential ({d})')
...
with open('DPT_results.txt', 'w') as f_out:
    f_out.write("======================================================================\n")
    f_out.write("HASIL ANALISIS DEFORMATION POTENTIAL THEORY (DPT): ZrFCl\n")
